...
public final class HybridNativeMpv: HybridNativeMpvSpec {
    ...
    /// Stream URL + user-agent from the current `load()`. Read by the
    /// native VideoToolbox source to open its own libavformat context
    /// against the same origin.
    var nativeStreamUrl: String? { return currentStreamUrl }
    var nativeUserAgent: String? { return currentUserAgent }
    /// Current playback position in seconds, as last observed from mpv's
    /// `playback-time`. Used as the starting offset when the native
    /// source attaches mid-session (seek-into / resume-on-pip-return).
    var currentPlaybackSeconds: Double { return currentPosition }
